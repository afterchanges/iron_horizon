...
void UInventoryComponent::InitializeAdd(const TSubclassOf<UItemBase> BaseClass,
  ...
  if (ItemDataTable && !DesiredItemID.IsNone()) {
    ...
    InQuantity <= 0 ? ItemReference->SetQuantity(1)
...
void UInventoryComponent::InitializeDrop(UItemBase *ItemToDrop,
                                         const int32 InQuantity) {
...
UItemBase *UInventoryComponent::FindNextItemByID(UItemBase *ItemIn) const {
  if (ItemIn) {
    if (const TArray<TObjectPtr<UItemBase>>::ElementType *Result =
            InventoryContents.FindByKey(ItemIn)) {
      return *Result;
    }
  }
  return nullptr;
...
UItemBase *UInventoryComponent::FindNextPartialStack(UItemBase *ItemIn) const {
  if (const TArray<TObjectPtr<UItemBase>>::ElementType *Result =
          InventoryContents.FindByPredicate(
              [&ItemIn](const TObjectPtr<UItemBase> &InventoryItem) {
                return InventoryItem->ID == ItemIn->ID &&
                       !InventoryItem->IsFullItemStack();
              })) {
    return *Result;
  }
  return nullptr;
...
void UInventoryComponent::AddNewItem(UItemBase *Item, const int32 AmountToAdd) {
  ...
  if (ItemDataTable && !NewItem->ID.IsNone()) {
    const FItemData *ItemData =
        ItemDataTable->FindRow<FItemData>(NewItem->ID, NewItem->ID.ToString());
