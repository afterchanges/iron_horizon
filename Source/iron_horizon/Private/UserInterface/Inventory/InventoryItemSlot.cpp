...
void UInventoryItemSlot::NativeOnInitialized() {
  ...
  if (ToolTipClass) {
    UInventoryTooltip *ToolTip =
        CreateWidget<UInventoryTooltip>(GetWorld(), ToolTipClass);
...
FReply
UInventoryItemSlot::NativeOnMouseButtonDown(const FGeometry &InGeometry,
  ...
  if (DragItemVisualClass) {
    ...
    if (World) {
      const TObjectPtr<UDragItemVisual> DragVisual =
          CreateWidget<UDragItemVisual>(
              this, TSubclassOf<UUserWidget>(DragItemVisualClass));
      DragVisual->ItemIcon->SetBrushFromTexture(ItemReference->AssetData.Icon);
      DragVisual->ItemBorder->SetBrushColor(ItemBorder->GetBrushColor());
      DragVisual->ItemQuantity->SetText(
          FText::AsNumber(ItemReference->Quantity));

      UItemDragDropOperation *DragItemOperation =
