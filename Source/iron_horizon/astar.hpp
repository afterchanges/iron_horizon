...
struct HexTileNode {
  ...
  HexTileNode(const FIntPoint &GridPositionIndex, float GScore, float FScore,
              HexTileNode *Parent)
      : GridPositionIndex(GridPositionIndex), GScore(GScore), FScore(FScore),
        Parent(Parent) {}
};

TArray<AHexTile *>
AHexGridManager::GetNeighbors(const FIntPoint &GridPositionIndex) {
...
TArray<FIntPoint>
AHexGridManager::HexGridAStar(AHexTile *StartTile, AHexTile *EndTile,
  ...
  while (OpenSet.Num() > 0) {
    ...
    for (AHexTile *Neighbor : Neighbors) {
      ...
      float GScore =
          CurrentNode->GScore + 1;  // assuming all edges have a weight of 1
      float HScore =
          HexDistance(Neighbor->GridPositionIndex, EndTile->GridPositionIndex);
      float FScore = GScore + HScore;

      HexTileNode *NeighborNode =
          OpenSetMap.FindRef(Neighbor->GridPositionIndex);
      if (NeighborNode == nullptr) {
        NeighborNode = new HexTileNode(Neighbor->GridPositionIndex, GScore,
                                       FScore, CurrentNode);
