...
static void MapKey(UInputMappingContext *InputMappingContext,
                   UInputAction *InputAction, FKey Key, bool bNegate = false,
                   bool bSwizzle = false,
...
void APlayerCameraController::SetupInputComponent() {
  ...
  MapKey(PawnMappingContext, MoveAction, EKeys::SpaceBar, false, true,
         EInputAxisSwizzle::ZYX);
  MapKey(PawnMappingContext, MoveAction, EKeys::LeftShift, true, true,
         EInputAxisSwizzle::ZYX);

  RotateAction = NewObject<UInputAction>(this);
  RotateAction->ValueType = EInputActionValueType::Axis3D;
  MapKey(PawnMappingContext, RotateAction, EKeys::E, false, true,
         EInputAxisSwizzle::ZXY);
  MapKey(PawnMappingContext, RotateAction, EKeys::Q, true, true,
         EInputAxisSwizzle::ZXY);
  ...
  InputComponent->BindKey(EKeys::J, IE_Pressed, this,
                          &APlayerCameraController::OnJKeyPressed);
  ...
  InputComponent->BindKey(EKeys::P, IE_Pressed, this,
                          &APlayerCameraController::OnPKeyPressed);
}
...
void APlayerCameraController::OnJKeyPressed() {
  ...
  if (GetMousePosition(MouseX, MouseY)) {
    ...
    if (GetHitResultAtScreenPosition(FVector2D(MouseX, MouseY), ECC_Visibility,
      ...
      if (HexTile) {
        ...
        UE_LOG(LogTemp, Warning, TEXT("J MOUSE POSITION: %s"),
        ...
        if (!AHexTile::StartTile) {
          ...
          if (!HexGridManager) {
            UE_LOG(LogTemp, Warning,
                   TEXT("Failed to find HexGridManager instance"));
            return;
          }
          ...
          if (HexGridManager) {
            ...
            UE_LOG(LogTemp, Warning, TEXT("Shortest path length: %d"),
                   Path.Num());
            ...
            UE_LOG(LogTemp, Warning,
                   TEXT("Failed to get HexGridManager instance"));
          }
        }
      } else {
