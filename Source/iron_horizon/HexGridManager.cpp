  ...
  for (int32 y = 0; y < GridHeight; ++y) {
    for (int32 x = 0; x < GridWidth; ++x) {
      HexTileType spawnTileType =
          terrainTilesByHeight[heightTileGroupDecider(noise_height_map[x][y])]
                              [0];
      float newTileHeightAdjusted =
          FMath::RandRange(0.f, 1.f) * (TileHeightRanges[spawnTileType][1] -
                                        TileHeightRanges[spawnTileType][0]) +
          TileHeightRanges[spawnTileType][0];

      newTileHeightAdjusted =
          noise_height_map[x][y] >= 0.0f ? noise_height_map[x][y] : 0.0f;
      ...
      if (TileToSpawn) {
        ...
        if (World) {
          ...
          UE_LOG(LogTemp, Warning, TEXT("Failed to spawn actor at (%d, %d)"), x,
                 y);
        }
      } else {
        UE_LOG(LogTemp, Warning, TEXT("No class specified for tile type %d"),
               (int32)spawnTileType);
      }
    }
  }
        ...
        if (tile) {
          ...
          if (World) {
            ...
            if (NewTile) {
              ...
              UE_LOG(LogTemp, Warning, TEXT("City at (%d, %d)"), city.X,
                     city.Y);
            }
          }
        }
